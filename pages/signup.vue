<template>
  <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 via-blue-50 to-white">
    <form
      ...
      class="bg-white p-8 rounded-2xl shadow-xl w-full max-w-xs animate-fadein border border-blue-100"
    >
      <div class="flex flex-col items-center gap-2 mb-6">
        ...
        <h2 class="text-2xl font-extrabold text-gray-700 text-center tracking-tight">Create your account</h2>
      </div>
      <label class="block mb-2 text-gray-600 text-sm font-semibold">Username</label>
      <input
        v-model="username"
        required
        placeholder="Username"
        class="mb-4 px-3 py-2 border rounded-lg w-full focus:ring-2 focus:ring-blue-200 focus:outline-none transition"
      ...
      <div class="relative mb-3">
        <input
          ...
          class="px-3 py-2 border rounded-lg w-full focus:ring-2 focus:ring-blue-200 focus:outline-none transition pr-10"
          autocomplete="new-password"
        />
        <button
          type="button"
          class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-blue-500 transition"
      ...
      <button
        type="submit"
        class="w-full bg-gradient-to-r from-blue-400 to-blue-600 hover:from-blue-500 hover:to-blue-700 text-white py-2 rounded-lg font-bold shadow-lg transition transform hover:-translate-y-0.5 active:scale-95 mt-3"
      >Signup</button>
      <div v-if="error" class="text-red-500 text-sm mt-2 text-center animate-shake">{{ error }}</div>
      <div class="text-center mt-5">
        <NuxtLink
          to="/login"
          class="text-blue-600 hover:underline font-semibold transition"
        >Already have an account? <span class="underline">Login</span></NuxtLink>
      </div>
    </form>
  </div>
</template>
